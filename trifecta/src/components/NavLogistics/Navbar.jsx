...
const NavBar = () => {
  ...
  const Menus = [
    { title: 'Dashboard', icon: <HiOutlineViewBoards />, path: '/logistics-dashboard' },
    {
    ...
    { title: 'Go Back', icon: <HiOutlineLogout />, path: '/dashboard' },
  ]

  return (
    <div className="hidden md:block z-50 h-screen">
      <div
        className={` ${
          open ? 'w-72' : 'w-20 '
      ...
      >
        ...
        <ul className="pt-[2rem]">
          {Menus.map((Menu, index) => (
            <li
              key={index}
              className={`flex  rounded-md  p-2 cursor-pointer  text-gray-300 text-sm items-center gap-x-4 md:text-base hover:bg-gradient-color transition-all duration-300
            ${Menu.gap ? 'mt-12' : 'mt-2'} ${
